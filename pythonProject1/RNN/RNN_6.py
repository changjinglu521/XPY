...
seq_length=100          #句子最大长度
vocab_dim=128          #词的emedding维度
num_classes=2          #分类类别
...
model.add(layers.Embedding(output_dim=vocab_dim, input_dim=vocab_size, input_length=seq_length,mask_zero=True))
...
model.add(layers.LSTM(100, dropout=0.3, recurrent_dropout=0.3))
...
history=model.fit(train_x,train_y,validation_split=0.1,epochs=5,batch_size=64,verbose=1)
plt.plot(history.history['val_acc'])
plt.plot(history.history['val_loss'])
plt.legend(['training', 'valiation'], loc='upper left')
#plt.savefig("rnn_6.png")
plt.show()
